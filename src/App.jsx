...
import { 
  ...
  Image as ImageIcon, X, Lock, Download, FileDown, Hash, Key,
      ...
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl relative animate-in slide-in-from-bottom-10 fade-in duration-300">
        ...
        <div className="flex flex-col items-center text-center mt-2">
          ...
          {isIOS ? (
            ...
            <button onClick={handleInstallClick} className="w-full bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-bold py-4 px-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg transition-all">
              <Download className="w-6 h-6" /> 立即安裝至桌面
            </button>
          )}
...
export default function App() {
  ...
  // 優化：直接在初始化時判斷並寫入記憶，避免畫面閃爍
  const [isAdminMode, setIsAdminMode] = useState(() => {
    if (typeof window !== 'undefined') {
      const urlHasAdmin = window.location.href.includes('admin=true');
      if (urlHasAdmin) {
        localStorage.setItem('isWefixOwner', 'true');
        return true;
      }
      return localStorage.getItem('isWefixOwner') === 'true';
    }
    return false;
  });
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-20 md:pb-0">
      ...
      <div className="bg-white shadow-sm px-4 py-3 flex justify-between items-center fixed top-0 w-full z-50 md:relative">
        ...
        {isAdminMode && (
          <div className="flex gap-2 bg-slate-100 p-1 rounded-lg">
            <button onClick={() => setView('form')} className={`px-3 py-1 text-sm rounded-md transition-colors ${view === 'form' ? 'bg-white shadow-sm text-blue-600 font-bold' : 'text-slate-500'}`}>
              客戶報修
            </button>
            <button onClick={handleAdminViewClick} className={`px-3 py-1 text-sm rounded-md transition-colors ${view === 'admin' || view === 'login' ? 'bg-white shadow-sm text-blue-600 font-bold' : 'text-slate-500'}`}>
              後台管理
            </button>
          </div>
        )}
      ...
      <div className="pt-16 md:pt-6">
        ...
        {view === 'login' && <AdminLogin onLoginSuccess={() => {setIsAdminAuthenticated(true); setView('admin');}} onCancel={() => setView('form')} />}
...
function AdminLogin({ onLoginSuccess, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(false);
    try {
      const adminDocRef = doc(db, 'system_settings', 'admin');
      const docSnap = await getDoc(adminDocRef);
      let currentPassword = ADMIN_PASSWORD;
      if (docSnap.exists() && docSnap.data().password) {
        currentPassword = docSnap.data().password;
      }
      if (password === currentPassword) onLoginSuccess();
    ...
    <div className="flex flex-col items-center p-4 mt-10">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl overflow-hidden p-8 text-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="button" onClick={onCancel} className="w-full text-gray-500 text-sm hover:text-gray-700 py-2">返回客戶報修頁面</button>
        </form>
      </div>
    </div>
  );
...
function RepairForm({ user }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!user || isOverLimit) return;
    
    const { name, phone, address, description } = formData;
    if (!name.trim()) { setErrorMsg('「聯絡人姓名」為必填，請輸入您的姓名。'); return; }
    if (!phone.trim()) { setErrorMsg('「聯絡電話」為必填，請輸入您的電話號碼。'); return; }
    const cleanPhone = phone.replace(/[-\s]/g, '');
    const isMobile = /^09\d{8}$/.test(cleanPhone);
    const isLandline = /^0[2-8]\d{7,8}(?:#\d+)?$/.test(cleanPhone);
    if (!isMobile && !isLandline) { setErrorMsg('電話格式不正確！請輸入有效的「手機號碼」(10碼) 或「市話」(需含區碼，如 04-23456789)。'); return; }
    if (!address.trim()) { setErrorMsg('「維修地址」為必填，請提供現場地址。'); return; }
    if (!description.trim()) { setErrorMsg('「故障狀況描述」為必填，請簡述現場狀況。'); return; }
    ...
    try {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        if (data.caseNumber && data.caseNumber.startsWith(todayPrefix)) todayCount++;
      });
  ...
  const handleReset = () => {
    ...
    setImages([]); setStatus('idle'); setErrorMsg(''); setSubmittedCaseNumber('');
  };
    ...
    return (
      <div className="flex items-center justify-center p-4 mt-10">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 text-center animate-in fade-in zoom-in duration-300 border-t-8 border-green-500">
          ...
          <p className="text-gray-600 mb-6 leading-relaxed">新峰聚工程的維修團隊已收到您的需求，並記錄至系統。我們將盡快安排工程人員前往處理。</p>
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-8">
            <p className="text-sm text-gray-500 font-semibold mb-1 flex items-center justify-center gap-1"><Hash className="w-4 h-4"/> 您的案件編號</p>
            <p className="text-2xl font-mono font-bold text-blue-700 tracking-wider">{submittedCaseNumber}</p>
            <p className="text-xs text-gray-400 mt-2">您可以截圖保留此編號，方便日後查詢</p>
          </div>
          <button onClick={handleReset} className="w-full bg-slate-100 text-slate-700 font-semibold py-3 px-4 rounded-xl hover:bg-slate-200 transition-colors active:scale-95">返回重新報修</button>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden">
        <div className="bg-gradient-to-r from-blue-700 to-blue-900 px-6 py-8 text-white relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-start gap-4">
            ...
            <div>
              ...
              <h1 className="text-3xl font-bold flex items-center gap-2">快速線上報修</h1>
              <p className="mt-2 text-blue-100 text-sm opacity-90">掃描 QR Code 填寫，工程團隊將收到雲端通知</p>
            </div>
          </div>
          {source && <div className="mt-4 inline-block bg-white text-blue-800 text-xs font-bold px-3 py-1 rounded-full shadow-sm">目前分店：{source}</div>}
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">聯絡人姓名 <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">聯絡電話 <span className="text-red-500">*</span></label>
              <div className="relative">
          ...
          <div>
            ...
            <div className="relative">
              <div className="absolute top-3 left-3 pointer-events-none"><MapPin className="h-5 w-5 text-gray-400" /></div>
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">故障狀況描述 (報修項目) <span className="text-red-500">*</span></label>
            <div className="relative">
          ...
          <div>
             <div className="flex justify-between items-end mb-2">
               ...
               <span className={`text-xs ${isOverLimit ? 'text-red-500 font-bold' : 'text-gray-400'}`}>{images.length} / {MAX_IMAGES}</span>
             </div>
               ...
               <div className="mt-3 grid grid-cols-3 gap-2">
                 {images.map((img, index) => (
                   <div key={index} className={`relative aspect-square rounded-lg overflow-hidden border ${index >= MAX_IMAGES ? 'border-red-500 opacity-50' : 'border-slate-200'} group bg-slate-100 animate-in zoom-in duration-200`}>
                     ...
                     <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 bg-black bg-opacity-50 text-white rounded-full p-1 opacity-80 hover:opacity-100 hover:bg-red-500 transition-all"><X className="w-3 h-3" /></button>
          ...
          <div className="mt-16 pt-10 border-t-2 border-dashed border-slate-200 relative">
            <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-white px-4 text-xs font-bold text-slate-400 tracking-widest">表單結束</div>
            {isOverLimit && (
              <div className="mb-4 flex items-center justify-center gap-2 text-red-600 font-bold bg-red-50 py-3 px-4 rounded-xl animate-pulse">
                <AlertCircle className="w-5 h-5 shrink-0" /> <span>照片超過五張，無法傳送</span>
              </div>
            )}
            <button type="submit" disabled={status === 'submitting' || isOverLimit} className={`w-full flex items-center justify-center gap-2 py-4 px-4 rounded-xl shadow-md text-xl font-bold text-white transition-all ${isOverLimit ? 'bg-slate-300 cursor-not-allowed shadow-none' : status === 'submitting' ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 hover:shadow-lg active:scale-95'}`}>
...
function AdminDashboard({ user }) {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      setTimeout(() => { setShowPasswordModal(false); setNewPassword(''); setPasswordMsg(''); }, 1500);
    } catch (error) {
  ...
  const prepareExportExcel = () => {
    ...
    const statusTextMap = { 'pending': '待處理', 'processing': '處理中', 'completed': '已完成' };
    ...
    const rows = reports.map(report => {
      ...
      return [
        escapeCSV(report.caseNumber ? report.caseNumber : '無編號'),
        escapeCSV(report.source || '一般網頁'),
        escapeCSV(statusTextMap[report.status || 'pending']),
        escapeCSV(dateStr),
    ...
    setDownloadModal({ url, fileName: `新峰聚工程_報修紀錄_${dateString}.csv` });
  };
  ...
  const getStatusSelectStyle = (status) => {
    switch (status) {
      ...
      case 'pending': default: return 'bg-red-100 text-red-800 border-red-200 focus:ring-red-500';
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 relative">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2"><List className="w-6 h-6 text-blue-600" />報修案件管理</h2>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={() => setShowPasswordModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-bold bg-slate-200 text-slate-700 hover:bg-slate-300 transition-all"><Key className="w-4 h-4" /> 更改密碼</button>
          <button onClick={prepareExportExcel} disabled={reports.length === 0} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${reports.length === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 text-white shadow-sm hover:shadow-md active:scale-95'}`}><Download className="w-4 h-4" /> 匯出 Excel</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {reports.map((report) => {
            ...
            return (
              <div key={report.id} className={`bg-white rounded-2xl shadow-sm border p-5 transition-all duration-500 ${isCompleted ? 'border-gray-100 opacity-60 grayscale-[15%]' : 'border-gray-200 hover:shadow-md'}`}>
                <div className="flex flex-col gap-4">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center flex-wrap gap-3 mb-2">
                        <div className="relative inline-block shrink-0">
                          <select value={currentStatus} onChange={(e) => handleStatusChange(report.id, e.target.value)} className={`appearance-none text-sm font-bold pl-3 pr-8 py-1.5 rounded-md cursor-pointer outline-none border transition-colors focus:ring-2 ${getStatusSelectStyle(currentStatus)}`}>
                            <option value="pending">🔴 待處理</option>
                            <option value="processing">⚡ 處理中</option>
                            <option value="completed">✅ 已完成</option>
                          </select>
                          <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-current opacity-70"><ChevronDown className="w-4 h-4" /></div>
                        </div>
                        <h3 className={`font-bold text-lg ${isCompleted ? 'text-gray-500 line-through' : 'text-gray-800'}`}>{report.name}</h3>
                      </div>
                      <div className="flex flex-col sm:flex-row sm:flex-wrap gap-x-6 gap-y-2 text-sm text-gray-600 mb-3">
                        ...
                        {report.address && <span className="flex items-start gap-1"><MapPin className="w-4 h-4 text-gray-400 shrink-0 mt-0.5"/> {report.address}</span>}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1 ml-2 shrink-0">
                      {report.caseNumber && <div className={`text-xs font-bold px-2 py-1 rounded-md border ${isCompleted ? 'text-gray-500 bg-gray-50 border-gray-200' : 'text-blue-700 bg-blue-50 border-blue-100'}`}>案件編號 {report.caseNumber}</div>}
                      {report.source && report.source !== '一般網頁' && <div className="text-xs font-bold px-2 py-1 rounded-md bg-purple-100 text-purple-700 mt-1">來源：{report.source}</div>}
                      <div className="text-sm text-gray-400 flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-md mt-1"><Clock className="w-4 h-4" /><span>{formatDate(report.createdAt)}</span></div>
                    </div>
                  </div>
                  <div className={`p-3 rounded-lg text-sm border ${isCompleted ? 'bg-gray-50 border-gray-100 text-gray-500' : 'bg-slate-50 border-slate-100 text-gray-700'}`}>
                    <span className="font-semibold block text-xs text-gray-500 mb-1">報修項目 / 狀況描述：</span>{report.description}
                  </div>
                  {imagesList.length > 0 && (
                    <div className="mt-2">
                      <span className="text-xs text-gray-500 font-semibold mb-2 flex items-center gap-1"><ImageIcon className="w-3 h-3" /> 現場照片 ({imagesList.length}張)</span>
                      <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                        {imagesList.map((img, idx) => (
                          <div key={idx} className="cursor-pointer group relative rounded-lg overflow-hidden border border-slate-200 shadow-sm shrink-0 w-24 h-24 sm:w-28 sm:h-28" onClick={() => setSelectedImageIndex({ reportId: report.id, imgIndex: idx, images: imagesList })}>
                            <img src={img} alt="現場照片" className={`w-full h-full object-cover transition-transform ${isCompleted ? '' : 'group-hover:scale-105'}`} />
                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 flex items-center justify-center transition-all"><ImageIcon className="text-white opacity-0 group-hover:opacity-100 w-6 h-6" /></div>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-[120] bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full relative">
            <button onClick={() => setShowPasswordModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2"><Key className="w-5 h-5 text-blue-600" /> 更改後台密碼</h3>
            <form onSubmit={handleUpdatePassword}>
              <input type="text" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="請輸入新密碼" className="w-full bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 py-3 px-4 outline-none mb-4 tracking-widest text-center" autoFocus />
              {passwordMsg && <p className="text-sm font-bold text-center mb-4 text-green-600">{passwordMsg}</p>}
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl">確認修改</button>
            </form>
          </div>
        </div>
      )}
      ...
      {downloadModal && (
        <div className="fixed inset-0 z-[110] bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center relative animate-in fade-in zoom-in duration-200">
            <button onClick={() => setDownloadModal(null)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><X className="w-6 h-6" /></button>
            <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><FileDown className="w-8 h-8 text-green-600" /></div>
            <h3 className="text-xl font-bold text-gray-800 mb-2">檔案已準備就緒</h3>
            <p className="text-gray-500 text-sm mb-6">點擊下方按鈕即可將報表儲存至您的裝置。</p>
            <a href={downloadModal.url} download={downloadModal.fileName} onClick={() => {setTimeout(() => setDownloadModal(null), 1000);}} className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-xl transition-colors"><Download className="w-5 h-5" /> 點擊下載 Excel 檔</a>
          </div>
        </div>
      )}
      ...
      {selectedImageIndex && (
        <div className="fixed inset-0 z-[100] bg-black bg-opacity-90 flex items-center justify-center p-4">
          <div className="relative w-full h-full flex flex-col items-center justify-center">
            <button className="absolute top-4 right-4 sm:top-10 sm:right-10 text-white hover:text-gray-300 z-50 bg-black bg-opacity-50 rounded-full p-2" onClick={closeImageViewer}><X className="w-8 h-8" /></button>
            <img src={selectedImageIndex.images[selectedImageIndex.imgIndex]} alt="放大照片" className="max-w-full max-h-[80vh] object-contain" />
            {selectedImageIndex.images.length > 1 && (
              <div className="absolute bottom-10 flex items-center gap-4 bg-black bg-opacity-50 px-4 py-2 rounded-full">
                <button className={`text-white p-1 rounded-full ${selectedImageIndex.imgIndex === 0 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-white hover:bg-opacity-20'}`} onClick={(e) => { e.stopPropagation(); if(selectedImageIndex.imgIndex > 0) setSelectedImageIndex(prev => ({ ...prev, imgIndex: prev.imgIndex - 1 })); }}><ChevronRight className="w-8 h-8 rotate-180" /></button>
                <span className="text-white font-mono">{selectedImageIndex.imgIndex + 1} / {selectedImageIndex.images.length}</span>
                <button className={`text-white p-1 rounded-full ${selectedImageIndex.imgIndex === selectedImageIndex.images.length - 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-white hover:bg-opacity-20'}`} onClick={(e) => { e.stopPropagation(); if(selectedImageIndex.imgIndex < selectedImageIndex.images.length - 1) setSelectedImageIndex(prev => ({ ...prev, imgIndex: prev.imgIndex + 1 })); }}><ChevronRight className="w-8 h-8" /></button>
              </div>
            )}
          </div>
        </div>
